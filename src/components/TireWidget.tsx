...
export default function TireWidget({ settings, odo, setActiveTab }: TireWidgetProps) {
  if (!settings.frontTire && !settings.rearTire) {
    return (
      <div className="glass-panel" style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '8px', textAlign: 'center' }}>
        <h3 style={{ margin: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
          <img src={`${import.meta.env.BASE_URL}bike-icon.jpg`} alt="Bike" style={{ width: '120px', height: 'auto', objectFit: 'contain', filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.5))' }} /> 
  ...
  return (
    <div className="glass-panel" style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
      <h3 style={{ margin: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
        <img src={`${import.meta.env.BASE_URL}bike-icon.jpg`} alt="Bike" style={{ width: '120px', height: 'auto', objectFit: 'contain', filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.5))' }} /> 
        <span>Stan Opon</span>
      </h3>
